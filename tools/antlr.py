...
def AntlrFileEmitter(target, source, env):
    ...
    for src in source:
        ...
        path = src.target_from_source('', '').get_dir().get_path() + '/'
    ...
    targetSet = set()
    for p in parsers:
        targetSet.add(path + p['class'] + '.hpp')
        targetSet.add(path + p['class'] + '.cpp')
        targetSet.add(path + p['vocab'] + 'TokenTypes.hpp')
        targetSet.add(path + p['vocab'] + 'TokenTypes.txt')
        ...
        if p['superclass'] not in ['Lexer', 'Parser', 'TreeParser']:
            ...
            if pFile != _GetFileForParser(p['superclass'], env):
                (path, sep, basename) = str(pFile).rpartition('/')
...
def AntlrFileActionGenerator(source, target, env, for_signature):
    dependencies = _GetAntlrFileDependencies(source[0], env)
    if len(dependencies) > 0:
        ...
        includes = ''
    return '$ANTLR -o "%s" %s "%s"' % (target[0].get_dir(), includes, source[0])
